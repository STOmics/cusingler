...
/*
    ...
    batch: cell_len
*/
__global__ void rankdata_batch(uint16* dataIn,float* dataOut,const int datalen,const int batch)
{
...
__global__ void rankdata_bin3_pitch(uint16* qry, const size_t pitch,const uint32 cols, const uint32 rows, float* res)
{
...
vector<uint32> get_label(InputData& rawdata,int mod)
{
    ...
    for (int i = 0; i < qry_height; ++i)
    {
        ...
        get_device_qry_line<<< h_gene_idx.size()/1024 + 1, 1024 >>>(d_gene_idx, d_qry, h_gene_idx.size(), qry_width, d_qry_line);
        err = cudaGetLastError();
        if (err != cudaSuccess )
        {
...
vector<uint32> finetune(int mod)
{
    Timer timer("ms");
    // process each cell
    vector<uint32> res;
